  ...
  try {
    ...
    if (req.method === "PUT") {
      ...
      if (typeof data.is_approved !== "boolean")
        return error(res, "is_approved boolean required", 400);
      const { rows } = await query(
        `UPDATE wisdomintech.comments SET is_approved=$2, updated_at=now() WHERE id=$1 RETURNING *`,
        [id, data.is_approved],
      );
    ...
    if (req.method === "DELETE") {
      if (!id) return error(res, "id is required", 400);
      const { rows } = await query(
        `DELETE FROM wisdomintech.comments WHERE id=$1 RETURNING id`,
        [id],
      );
      if (!rows.length) return error(res, "Not found", 404);
