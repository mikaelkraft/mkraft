...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      return json(res, {
        role: user.role,
        publisher_request_status: user.publisher_request_status,
        banned: user.banned,
    ...
    if (req.method === "POST") {
      ...
      if (user.publisher_request_status === "pending") {
        return json(
          res,
          { role: user.role, publisher_request_status: "pending" },
          200,
        );
      }
      await query(
        `UPDATE wisdomintech.user_profiles
      ...
      return json(
        res,
        { role: "viewer", publisher_request_status: "pending" },
        201,
      );
    }
    return error(res, "Method not allowed", 405);
  } catch (e) {
    return error(res, "Failed to process publisher request", 500, {
      detail: e.message,
    });
  }
};
