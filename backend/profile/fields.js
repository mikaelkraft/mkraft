...
module.exports = async function handler(req, res) {
  try {
    if (req.method === "GET") {
      ...
      const sql =
        isAdmin && allParam
          ? "SELECT * FROM wisdomintech.profile_field_rules ORDER BY field_name"
          : "SELECT * FROM wisdomintech.profile_field_rules WHERE enabled = true ORDER BY field_name";
    ...
    if (req.method === "POST") {
      ...
      if (!body || !Array.isArray(body.rules))
        return error(res, "rules array required", 400);
      const results = [];
      for (const r of body.rules) {
        if (!r.field_name) continue;
        const { rows } = await query(
          `INSERT INTO wisdomintech.profile_field_rules (field_name, enabled, editable_roles, required)
          ...
          [
            r.field_name,
            r.enabled !== false,
            r.editable_roles || ["admin", "publisher", "viewer"],
            r.required === true,
          ],
        );
        results.push(rows[0]);
    ...
    return error(res, "Failed to process field rules", 500, {
      detail: e.message,
    });
  }
};
