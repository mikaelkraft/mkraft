...
async function getUserWithRole(req) {
  ...
  try {
    const { rows } = await query(
      `SELECT id, email, role, publisher_request_status,
                     banned, ban_reason, warning_count, last_warning_at, banned_at
                  FROM wisdomintech.user_profiles WHERE id = $1`,
      [user.id],
    );
    ...
    if (adminEmail && user.email === adminEmail && role !== "admin") {
      await query(
        "UPDATE wisdomintech.user_profiles SET role = 'admin', role_updated_at = now() WHERE id = $1",
        [user.id],
      );
      role = "admin";
    }
