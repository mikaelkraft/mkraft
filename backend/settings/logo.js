...
module.exports = async function handler(req, res) {
  try {
    ...
    url = url.trim();
    if (!/^https?:\/\//i.test(url))
      return error(res, "url must be absolute http(s)", 400);
    ...
    if (existing.rows.length) {
      const { rows } = await query(
        "UPDATE wisdomintech.site_settings SET logo_url=$1, site_logo_url=$1, updated_at=now() WHERE id=$2 RETURNING logo_url, site_logo_url",
        [url, existing.rows[0].id],
      );
      return json(res, {
        success: true,
        logo_url: rows[0].logo_url,
        site_logo_url: rows[0].site_logo_url,
      });
    } else {
      const { rows } = await query(
        "INSERT INTO wisdomintech.site_settings (logo_url, site_logo_url) VALUES ($1,$1) RETURNING logo_url, site_logo_url",
        [url],
      );
      return json(
        res,
