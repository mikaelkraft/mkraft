...
module.exports = async function handler(req, res) {
  try {
    ...
    if (search) {
      conditions.push(
        `(title ILIKE $${i} OR excerpt ILIKE $${i + 1} OR content ILIKE $${i + 2})`,
      );
      const like = `%${search}%`;
    ...
    if (req.method === "POST") {
      ...
      const { rows: flagRows } = await query(
        "SELECT enabled FROM wisdomintech.feature_flags WHERE flag_key = $1",
        ["publisher_program"],
      );
      ...
      if (user.banned) {
        return error(res, "Account banned", 403, {
          reason: user.ban_reason || "policy_violation",
        });
      }
      ...
      if (body.content) {
        const { rows: settings } = await query(
          "SELECT enable_video FROM wisdomintech.site_settings ORDER BY created_at DESC NULLS LAST LIMIT 1",
        );
      ...
      if (!body.featured_image && body.content) {
        const imgMatch = body.content.match(
          /<img[^>]*src=["']([^"'>]+)["'][^>]*>/i,
        );
      ...
      const params = [
        ...
        body.excerpt || null,
        body.content || null,
        body.featured_image || autoFeatured || null,
    ...
    if (req.method === "PUT") {
      ...
      try {
        const existingRes = await query(
          "SELECT * FROM wisdomintech.blog_posts WHERE id = $1",
          [id],
        );
        ...
        console.error("revision_capture_failed", {
          id,
      ...
      if ("content" in body) {
        const { rows: settings2 } = await query(
          "SELECT enable_video FROM wisdomintech.site_settings ORDER BY created_at DESC NULLS LAST LIMIT 1",
        );
      ...
      if ("featured_image" in body)
        setField("featured_image", body.featured_image);
