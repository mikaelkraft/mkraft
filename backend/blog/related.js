...
module.exports = async function handler(req, res) {
  try {
    ...
    const { rows: targetRows } = await query(
      `SELECT id, tags FROM wisdomintech.blog_posts WHERE slug = $1 AND status = 'published' LIMIT 1`,
      [slug],
    );
    ...
    if (related.length < 3) {
      ...
      if (keywords.length) {
        const likeClauses = keywords
          .map((_, i) => `(title ILIKE $${i + 2} OR excerpt ILIKE $${i + 2})`)
          .join(" OR ");
    ...
    related.sort((a, b) => {
      const aOverlap = (a.tags || []).filter((t) =>
        tagArray.includes(t),
      ).length;
      const bOverlap = (b.tags || []).filter((t) =>
        tagArray.includes(t),
      ).length;
    ...
    return error(res, "Failed to load related posts", 500, {
      detail: e.message,
    });
  }
};
