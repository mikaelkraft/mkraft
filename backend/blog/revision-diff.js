...
module.exports = async function handler(req, res) {
  try {
    ...
    const truncate = (s) =>
      s && s.length > 50000 ? s.slice(0, 50000) + "\n...[truncated]" : s;
    return json(res, {
      before: {
        title: postRes.rows[0].title,
        content: truncate(postRes.rows[0].content || ""),
        excerpt: postRes.rows[0].excerpt,
      },
      after: {
        title: rev.title,
        content: truncate(rev.content || ""),
        excerpt: rev.excerpt,
      },
      meta: {
        revision_id: rev.id,
        blog_post_id: rev.blog_post_id,
