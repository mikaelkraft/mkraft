...
module.exports = async function handler(req, res) {
  try {
    ...
    const { rows } = await query(
      `SELECT id, updated_at, created_at FROM wisdomintech.projects WHERE status='published' ORDER BY updated_at DESC NULLS LAST LIMIT 2000`,
    );
    ...
    res.end("projects sitemap generation failed");
  }
};
