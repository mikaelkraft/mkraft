...
const BlogManagement = ({
  ...
  const filteredPosts = (blogPosts || [])
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "comments")
        return ((a?.comments || 0) - (b?.comments || 0)) * modifier;
      return 0;
    });
  ...
  const submitCreate = async (e) => {
    ...
    const words = (createForm.content || "")
      .trim()
      .split(/\s+/)
      ...
      <td className="px-4 py-4">
        <div className="flex flex-wrap gap-1">
          {(post?.tags || []).slice(0, 2).map((tag, index) => (
            <span
              key={index}
              className="px-2 py-1 bg-secondary/10 text-secondary text-xs rounded-full font-caption"
            >
              {tag}
            </span>
          ))}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {formErrors.slug && (
                <p className="mt-1 text-xs text-error">{formErrors.slug}</p>
              )}
            </div>
            <div>
              <label className="block text-sm text-text-secondary mb-1">
            ...
            <div className="md:col-span-2">
              <FileUpload
                ...
                onChange={(url) =>
                  setCreateForm({ ...createForm, featuredImage: url })
                }
                bucket="media"
            ...
            {formErrors.readTime && (
              <p className="mt-1 text-xs text-error">{formErrors.readTime}</p>
            )}
            <div>
              <label className="block text-sm text-text-secondary mb-1">
                Read Time (minutes)
              ...
              <Input
                ...
                onChange={(e) =>
                  setCreateForm({ ...createForm, readTime: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setCreateForm({ ...createForm, publishedAt: e.target.value })
                }
              />
            </div>
            <div className="md:col-span-2">
            ...
            await onPostUpdate(editingPost.id, {
              ...
              publishDate:
                ...
                (editForm.status === "published"
                  ? editingPost.publishDate || new Date().toISOString()
                  : null),
            });
            setEditingPost(null);
            setShowEditPreview(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <FileUpload
                ...
                onChange={(url) =>
                  setEditForm({ ...editForm, featuredImage: url })
                }
                bucket="media"
                pathPrefix="blog/featured"
                accept="image"
            ...
            <div>
              ...
              <Input
                type="number"
                min="1"
                value={editForm.readTime || 5}
                onChange={(e) =>
                  setEditForm({ ...editForm, readTime: e.target.value })
                }
              />
              {formErrors.readTime && (
                <p className="mt-1 text-xs text-error">{formErrors.readTime}</p>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, publishedAt: e.target.value })
                }
              />
            </div>
            <div className="md:col-span-2">
