    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          base + "/blog/revision-diff?revisionId=" + revisionId,
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background/90 backdrop-blur p-6 overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={onClose}
          className="px-3 py-1 rounded bg-error text-background text-sm"
        >
          Close
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto rounded border border-border-accent/30 bg-surface text-xs font-mono">
          ...
          <div
            ...
            className={
              r.type === "context"
                ? "flex px-2 py-0.5"
                : r.type === "add"
                  ? "flex px-2 py-0.5 bg-success/10"
                  : "flex px-2 py-0.5 bg-error/10"
            }
          >
        ...
        {!loading && diffRows.length === 0 && (
          <div className="p-4 text-text-secondary">No differences.</div>
        )}
      </div>
    </div>
  );
