...
export default function CommandPalette() {
  // Feature flag first – but we still must call hooks unconditionally.
  ...
  // Listener registered only if feature enabled to avoid unnecessary global listeners
  const listener = useCallback(
    (e) => {
      if (!enabled) return; // guard but keep hook ordering stable
  ...
  useEffect(() => {
    if (!enabled) return; // skip effect body if disabled
    window.addEventListener("keydown", listener);
    return () => window.removeEventListener("keydown", listener);
  }, [listener, enabled]);

  // We'll decide rendering later; never return early before hooks finished.
  const shouldRender = enabled; // simple flag for clarity
  ...
  groups.push({
    label: "Commands",
    items: scored.length ? scored : actionCandidates,
  });
  if (showPostSearch)
    groups.push({
      label: "Posts",
  ...
  useEffect(() => {
    ...
    if (cacheRef.current.has(key)) {
      setPostResults(cacheRef.current.get(key));
      return;
    }
    let aborted = false;
    const t = setTimeout(async () => {
      try {
        ...
        if (!aborted && res?.success) {
          const slice = res.data.slice(0, 5);
          cacheRef.current.set(key, slice);
          setPostResults(slice);
        }
      } finally {
  ...
  useEffect(() => {
    if (open) {
      ...
      return; // early exit only inside effect body
    }
    // Reset when closed
    setQuery("");
    setPostResults([]);
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-28">
          ...
          <div className="relative w-full max-w-xl mx-auto bg-surface border border-border-accent/30 rounded-xl shadow-xl overflow-hidden">
            ...
            <div className="max-h-80 overflow-y-auto" onKeyDown={onKeyDown}>
              ...
              {groups.map((g) => (
                <div key={g.label}>
                  <div className="px-4 pt-3 pb-1 text-[10px] uppercase tracking-wide text-text-secondary">
                    {g.label}
                    {g.label === "Posts" && loading ? " • Loading…" : ""}
                  </div>
                  <ul>
                    {g.items.map((item) => {
                      const idx = flat.findIndex(
                        (f) => f.id === item.id && f.group === g.label,
                      );
