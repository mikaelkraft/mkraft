...
export default function MarkdownField({
  ...
  const actions = [
    ...
    slashCommand("ul", "Bulleted List", () => `\n- Item 1\n- Item 2\n- Item 3\n`),
    slashCommand("img", "Image", () => `\n![alt text](https://example.com/image.png)\n`),
    slashCommand("link", "Link", () => `[title](https://example.com)`),
  ];
  const filteredActions = actions.filter((a) => a.cmd.startsWith(slashQuery.toLowerCase()));
  useEffect(() => {
  ...
  const handleKeyDown = (e) => {
    if (showSlash) {
      ...
      if (e.key === "ArrowUp") {
        e.preventDefault();
        setSlashIndex((i) => (i - 1 + filteredActions.length) % filteredActions.length);
        return;
      }
      if (e.key === "Enter") {
        e.preventDefault();
  ...
  return (
    <div className="markdown-field relative" onDragEnter={prevent} onDragOver={prevent} onDrop={handleDrop}>
