...
class SettingsService {
  ...
  async resetToDefaults() {
    try {
      const defaultSettings = {
        ...
        seo_settings: {
          keywords:
            "portfolio, developer, cyberpunk, react, javascript, web development",
  ...
  async setLogos({ light, dark }) {
    ...
    if (!token) return { success: false, error: "Authentication required" };
    const res = await fetch(
      (import.meta.env.VITE_API_BASE_URL || "/api") + "/settings/logo-upload",
      {
        method: "PUT",
        headers: {
          "content-type": "application/json",
  ...
  // Set OG default image (base64 or URL) via API
  async setOgImage(image) {
    if (!image) return { success: false, error: "No image provided" };
    const token = (await supabase.auth.getSession()).data.session?.access_token;
    if (!token) return { success: false, error: "Authentication required" };
    const res = await fetch(
      (import.meta.env.VITE_API_BASE_URL || "/api") + "/settings/og-upload",
      {
        method: "PUT",
        headers: {
          "content-type": "application/json",
