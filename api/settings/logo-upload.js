...
module.exports = async function handler(req, res) {
  try {
    ...
    async function handleVariant(kind, val) {
      ...
      const m = /^data:(image\/(?:png|jpeg|jpg|gif|webp));base64,(.+)$/i.exec(val);
    ...
    if (!lightUrl && !darkUrl) return error(res, 'No logo variants provided', 400);

    const existing = await query('SELECT id FROM wisdomintech.site_settings LIMIT 1');
    if (existing.rows.length) {
      const id = existing.rows[0].id;
      const { rows } = await query('UPDATE wisdomintech.site_settings SET logo_light_url = COALESCE($1, logo_light_url), logo_dark_url = COALESCE($2, logo_dark_url), updated_at = now() WHERE id = $3 RETURNING logo_light_url, logo_dark_url', [lightUrl, darkUrl, id]);
      return json(res, { success: true, ...rows[0] });
    } else {
      const { rows } = await query('INSERT INTO wisdomintech.site_settings (logo_light_url, logo_dark_url) VALUES ($1,$2) RETURNING logo_light_url, logo_dark_url', [lightUrl, darkUrl]);
