...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { rows: maxRow } = await query('SELECT COALESCE(MAX(display_order),0)+1 as next FROM wisdomintech.hero_slides');
      ...
      const { rows } = await query(insertSql, [body.title||null, body.subtitle||null, body.image_url||null, body.cta_label||null, body.cta_url||null, body.status||'published', body.display_order || next]);
    ...
    if (req.method === 'PUT') {
      ...
      const { rows } = await query(updateSql, [id, body.title??null, body.subtitle??null, body.image_url??null, body.cta_label??null, body.cta_url??null, body.status??null, body.display_order??null]);
