...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!Array.isArray(data.order)) return error(res, 'order array required', 400);
    ...
    try {
      for (const item of data.order) {
        ...
        await query(`UPDATE wisdomintech.hero_slides SET display_order=$2, updated_at=now() WHERE id=$1`, [item.id, item.display_order]);
      }
      await query('COMMIT');
    } catch (e) {
